...
pub async fn fetch_cursor_usage_csv(session_token: &str) -> Result<String> {
    ...
    {
        anyhow::bail!(
            "Cursor session expired. Please run 'token-stats cursor login' to re-authenticate."
        );
    }
    ...
    if let Some(ref label) = name {
        if find_account(label).is_some() {
            println!(
                "  {}",
                format!(
                    "Account '{}' already exists. Use 'token-stats cursor logout --name {}' first.",
                    label, label
                )
                .yellow()
            );
    ...
    let credentials = match credentials {
        ...
        None => {
            if let Some(ref account_name) = name {
                ...
                println!(
                    "{}",
                    "  Run 'token-stats cursor login' to authenticate.\n".bright_black()
                );
            }
            return Ok(());
        }
