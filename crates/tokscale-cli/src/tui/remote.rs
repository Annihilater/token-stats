...
mod tests {
    ...
    fn sample_stats(fetched_at_secs: u64) -> RemoteStats {
        RemoteStats {
            ...
            cached_for_api_url: "https://token-stats.com".to_string(),
        }
    }
    ...
    fn cache_round_trips_for_matching_account_and_server() {
        with_temp_config_dir(|| {
            ...
            let loaded = load_cached_remote_stats("alice", "https://token-stats.com")
    ...
    fn cache_normalizes_trailing_slash_in_api_url() {
        with_temp_config_dir(|| {
            ...
            assert!(load_cached_remote_stats("alice", "https://token-stats.com/").is_some());
        });
    }
    ...
    fn cache_rejects_stale_entries() {
        with_temp_config_dir(|| {
            ...
            save_remote_stats_cache(&stats).expect("save cache");
            assert!(load_cached_remote_stats("alice", "https://token-stats.com").is_none());
        });
    }
    ...
    fn cache_rejects_other_accounts_servers_and_anonymous_lookups() {
        with_temp_config_dir(|| {
            save_remote_stats_cache(&sample_stats(now_secs())).expect("save cache");
            assert!(load_cached_remote_stats("bob", "https://token-stats.com").is_none());
            assert!(load_cached_remote_stats("alice", "https://staging.token-stats.com").is_none());
            // Env-token sessions have no username; they must never trust cache.
            assert!(load_cached_remote_stats("", "https://token-stats.com").is_none());
        });
    }
